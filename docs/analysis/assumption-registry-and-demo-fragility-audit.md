...
| A-13 | ~~There is no `deauthorize_extension` function — extensions can only be replaced, never removed~~ **Corrected v0.0.19:** `revoke_extension_authorization` now exists on Gate, Turret, and SSU. Extensions CAN be removed (not just swapped). Blocked after `freeze_extension_config`. CivilizationControl does not use revocation — swap-only pattern still valid for our use case. | gate.move `revoke_extension_authorization` (v0.0.19) | Contract behavior | Medium | No | Once registered, gate always requires permit; beneficial stickiness |
...
| A-32 | ~~NO event emitted for `issue_jump_permit` from world-contracts~~ **Corrected v0.0.21:** `JumpPermitIssuedEvent` is now emitted by `issue_jump_permit` / `issue_jump_permit_with_id`. Full fields: `jump_permit_id`, `source_gate_id`, `source_gate_key`, `destination_gate_id`, `destination_gate_key`, `character_id`, `character_key`, `route_hash`, `expires_at_timestamp_ms`, `extension_type`. CC's current on-chain contract (compiled against world v1) does NOT emit this event — only effective after CC fresh publish against world v2. | gate.move `JumpPermitIssuedEvent` (v0.0.21) | Contract behavior | ~~Medium~~ Low | No | After v2a fresh publish, world-layer event supplements CC extension events |
