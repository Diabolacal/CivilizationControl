...
- Follow-ups: run one connected-wallet preview smoke on `/activity` and `/` when a usable wallet session is available so real wallet-scoped rows, category filters, and cursor pagination are proven in-browser; keep the remaining v1 custom posture/policy/gate-config history gaps explicitly deferred until operator demand justifies expanding the shared endpoint.
