...
- Result: human wallet smoke proved that `4f5bc0e` / `https://ac0cd430.civilizationcontrol.pages.dev` was still broken. The exact root cause was proof loss across two frontend seams: the selected-node `NodeLocalStructure` bucket merge in `src/lib/nodeDrilldownModel.ts` let weak display/fallback rows preserve canonical/display state while dropping authority-bearing `objectId`, `ownerCapId`, network-node context, action, and power fields; `normalizeIndexedActionCandidate` in `src/lib/operatorInventoryClient.ts` did not normalize live EF-Map top-level `actionCandidate.requiredIds` into per-action authority. The repair keeps operator inventory authoritative for object identity, owner-cap, network-node, action, and power proof; fallback rows may only enrich presentation. The new live diagnostic catches the screenshot condition directly: display/canonical row present, but Object ID/OwnerCap/Network Node/action/power proof missing. Fresh preview proof is `https://a187a67a.civilizationcontrol.pages.dev` with alias `https://fix-node-control-authority-r.civilizationcontrol.pages.dev`; both hosts returned `200` for `/`, `/activity`, `/nodes`, `/settings`, and `/dev/node-drilldown-lab`; unique preview HTML served `/assets/index-C8BAInCf.js` rather than the failed `index-Cwhp3oOh.js`; served JS assets contained `civilizationcontrol-sponsor` and `https://ef-map.com` while finding no `flappy-frontier-sponsor`, `ASSEMBLY_API_TOKEN`, `X-API-Key`, `SPONSOR_PRIVATE_KEY`, `CF_API_TOKEN`, or `CLOUDFLARE_ACCOUNT_ID`; and the fresh-preview live proof diagnostic returned `proofLoss.ok: true` for the main node with Amazing Assembler proof intact.
