...
## 2026-05-06 – Restore Node Control action authority
- Goal: repair the Node Control-only regression on `master` after the `feat/signal-history-display-parity` merge by restoring operator-inventory-first selected-node authority and power semantics, tightening the hot-path mount refetch that made warm entry slower, refreshing deterministic proof, and validating a fresh Pages preview without changing EF-Map/VPS behavior, Move contracts, package IDs, sponsor-worker behavior, or production deploy scope.
- Files: `src/lib/operatorInventoryAdapter.ts`, `src/hooks/useOperatorInventory.ts`, `scripts/check-operator-inventory-mapping.mts`, `scripts/check-node-power-live-usage.mts`, `scripts/check-structure-write-reconciliation.mts`, `docs/operations/network-node-drilldown-implementation-plan-20260501.md`, `docs/operations/node-control-write-action-audit-20260504.md`, `docs/operations/cc-read-path-to-efmap-indexer-replacement-plan-20260428.md`, `docs/operations/signal-feed-parity-audit-20260505.md`, `docs/decision-log.md`
- Diff: focused frontend authority or power merge repair, one hot-path query freshness correction, widened deterministic and live diagnostics, fresh preview proof, and tracked docs refresh
- Risk: medium - selected-node duplicate-row reconciliation and operator-inventory query freshness changed in the frontend model, but EF-Map/shared-backend authority, sponsor policy, Move/package state, vendor state, wallet builders, and production deploy scope did not
- Gates: `npm run typecheck` ✅ `npm run build` ✅ `git diff --check` ✅ `npx tsx scripts/check-signal-history-mapping.mts` ✅ `npx tsx scripts/check-operator-inventory-mapping.mts` ✅ `npx tsx scripts/check-node-drilldown-reconciliation.mts` ✅ `npx tsx scripts/check-structure-write-reconciliation.mts` ✅ `npx tsx scripts/check-node-power-capacity.mts` ✅ `npx tsx scripts/check-node-power-presets.mts` ✅ live authority or power diagnostic ✅ preview deploy ✅ unique+alias route smoke ✅ preview browser smoke on `/` and `/dev/node-drilldown-lab` ✅ served-bundle scan ✅
- Result: manual wallet smoke had found that Node Control alone regressed after the signal-history parity merge: attached-structure and node-self write actions disappeared, bulk child power controls disabled, and `Power usage unavailable` returned even though global list actions still worked. The root cause was frontend duplicate-row preference in `src/lib/operatorInventoryAdapter.ts`, which let display-fresher but weaker operator-inventory duplicates replace stronger rows wholesale and strip `ownerCapId`, action-candidate proof, wrapper-level node `powerUsageSummary`, and child `powerRequirement` before the selected-node model projected authority. The repair now merges duplicate node and child rows field-by-field so fresher display metadata can win presentation without stripping operator-inventory write authority or power data, while `useOperatorInventory()` now refetches on mount only when stale instead of forcing a warm-cache mount fetch. The widened live diagnostic for wallet `0x11dd567e72d160ad7116a7358684dfff800af2a8e429cd1a65778640f8a61f62` passed with zero authority failures, `Power 770 / 1000 GJ` on the active node, `Power 0 / 1000 GJ` on the offline node, enabled `Take all offline` only where valid, enabled `Bring all online` only where valid, and enabled preset save on both nodes. The fresh preview is `https://ac0cd430.civilizationcontrol.pages.dev` with alias `https://fix-node-control-authority-r.civilizationcontrol.pages.dev`; both hosts returned `200` on `/`, `/activity`, `/nodes`, `/settings`, and `/dev/node-drilldown-lab`, both served `/assets/index-Cwhp3oOh.js`, and served-bundle scanning across all 12 deployed JavaScript assets found `https://civilizationcontrol-sponsor.michael-davis-home.workers.dev` plus `https://ef-map.com` while finding no `flappy-frontier-sponsor`, `ASSEMBLY_API_TOKEN`, `X-API-Key`, `SPONSOR_PRIVATE_KEY`, `CF_API_TOKEN`, or `CLOUDFLARE_ACCOUNT_ID`. The only exact-case `Authorization` bundle hits remained user-facing `extension authorization` copy, not browser auth-header behavior. Integrated-browser wallet smoke was attempted on the preview, but the only exposed connector (`Slush`) still failed with `Connection failed`, so no wallet-backed preview write was executed in this environment.
- Follow-ups: use the fresh preview for manual wallet-connected smoke if desired; remaining gaps stay separate from this regression fix: historical rename rows are not backfilled, fresh CivilizationControl custom-package rows still need live action proof when new writes land, gate-specific live smoke remains limited until gates exist on the test wallet, and external marketplace coverage stays deferred.
