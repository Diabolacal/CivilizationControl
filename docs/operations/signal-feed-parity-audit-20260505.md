...
Status update (2026-05-06, Node Control authority regression repair on `fix/node-control-authority-regression`): the manual wallet smoke regression discovered after the `feat/signal-history-display-parity` merge was real, but it was not a Signal Feed contract regression. Node Control lost write actions, bulk controls, and power readouts because frontend duplicate-row preference in `src/lib/operatorInventoryAdapter.ts` let display-fresher but weaker operator-inventory duplicates replace stronger rows wholesale, stripping `ownerCapId`, action-candidate proof, wrapper-level `powerUsageSummary`, and child `powerRequirement` before the selected-node model projected authority. The repair now merges duplicate rows field-by-field so fresher display name and status can enrich presentation without stripping operator-inventory authority or power data, and the current Signal Feed display mappings remain intact. `scripts/check-operator-inventory-mapping.mts` plus `scripts/check-node-power-live-usage.mts` now fail if that authority-loss shape reappears. The warm-cache slowdown reduction also stayed outside the Signal Feed contract: `useOperatorInventory()` now uses stale-aware mount refetch, while Strategic Network still renders from root `useAssetDiscovery` and Signal Feed remains parallel dashboard enrichment rather than a topology blocker. Fresh preview proof is `https://ac0cd430.civilizationcontrol.pages.dev` with alias `https://fix-node-control-authority-r.civilizationcontrol.pages.dev`; route smoke passed on `/`, `/activity`, `/nodes`, `/settings`, and `/dev/node-drilldown-lab`, served-bundle scanning found the repo sponsor URL plus `https://ef-map.com`, and found no stale sponsor, `ASSEMBLY_API_TOKEN`, `X-API-Key`, or checked secret markers. No EF-Map contract, browser `queryEvents` boundary, Move/package-ID surface, sponsor-worker behavior, or production deploy state changed in this repair.
