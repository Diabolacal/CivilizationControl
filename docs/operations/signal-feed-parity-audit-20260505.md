...
The current normalizer flattens raw history rows into the older `SignalEvent` shape and discards several raw fields that matter for richer audit-log presentation:
...
That flattening boundary matters for parity work: EF-Map can add new rows inside the current route contract, but CivilizationControl will still need small display/mapping changes if the new rows depend on metadata such as `previousExtension`, `structureType`, `mode`, or `name`.
...
1. `SignalEvent` flattening drops raw `kind`, `severity`, `assemblyId`, `actorCharacterId`, and free-form `metadata`, so even correctly indexed rows will need small frontend mapping work for high-fidelity governance summaries.
2. Dashboard revenue uses `variant === "revenue"`, but the current `signalHistoryClient` maps `storage_deposit`, `storage_withdraw`, and `gate_transit` to informational variants. Revenue restoration is therefore a separate UI/display step, not part of the rename/power P0 fix.
...
4. The current UI does not expose the existing `networkNodeId`, `structureId`, or `since` query parameters even though the client contract already supports them.
5. No new probe script was added in this audit. The existing `scripts/check-signal-history-mapping.mts` still only exercises a narrow subset of kinds, so it does not yet prove rename, power, or governance-family parity.
