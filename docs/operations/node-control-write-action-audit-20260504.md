...
> Status update (2026-05-06, Node Control authority regression repair): manual wallet smoke on `master` after the `feat/signal-history-display-parity` merge found a real Node Control-only regression even though the general write builders still worked in global Turrets and Storages. The selected-node model was losing action authority and power inputs, so attached-structure right-click menus collapsed to no write actions, node-self actions disappeared, child bulk power buttons disabled, and `Power usage unavailable` returned. Root cause was frontend duplicate-row preference, not EF-Map or sponsor drift: `src/lib/operatorInventoryAdapter.ts` let display-fresher duplicate node and child rows replace stronger operator-inventory-backed rows wholesale, which stripped `ownerCapId`, action-candidate proof, wrapper-level `powerUsageSummary`, and child `powerRequirement` before the backend-membership selected-node model projected actions. The repair now merges duplicate rows field-by-field so fresher display name or status can win presentation without stripping operator-inventory write authority or power data. `scripts/check-operator-inventory-mapping.mts` now proves duplicate node plus duplicate child merges preserve authority and power, `scripts/check-node-power-live-usage.mts` now fails if operator-inventory authority is lost in the selected-node model, and live proof for wallet `0x11dd567e72d160ad7116a7358684dfff800af2a8e429cd1a65778640f8a61f62` showed zero authority failures with repaired bulk controls and power readouts (`Power 770 / 1000 GJ` and `Power 0 / 1000 GJ`). The same pass also narrowed the warm-cache slowdown by changing `useOperatorInventory()` from forced mount refetch to stale-aware `refetchOnMount: true`; Strategic Network still renders from root `useAssetDiscovery`, Signal Feed remains parallel dashboard enrichment rather than a topology blocker, and `node-assemblies` still stays fallback-only when operator inventory is missing or errored. Fresh preview proof is `https://ac0cd430.civilizationcontrol.pages.dev` with alias `https://fix-node-control-authority-r.civilizationcontrol.pages.dev`; route smoke passed on `/`, `/activity`, `/nodes`, `/settings`, and `/dev/node-drilldown-lab`; served-bundle scanning found the repo sponsor URL plus `https://ef-map.com` and no stale sponsor or secret/header markers; and deployed lab smoke confirmed visible action rails plus right-click `Hide from Node View`, `Bring Online`, and `Rename Assembly` with `title: null` on those controls. No EF-Map/VPS contract, Move/package-ID, sponsor-worker, browser `queryEvents`, broad-polling, or production-deploy change was made. Remaining gaps stay separate: historical rename rows are still not backfilled, fresh CivilizationControl custom-package rows still need live action proof when new writes land, gate-specific live smoke remains limited until gates exist on the test wallet, and external marketplace coverage remains deferred.

> Status update (2026-05-05, Signal Feed parity audit): the detailed read-only Signal Feed triage now lives in [signal-feed-parity-audit-20260505.md](signal-feed-parity-audit-20260505.md). This write-action audit still owns the zero-package action surface and sponsor-policy boundary; the dedicated parity audit now owns the current `signal-history.v1` coverage matrix, the P0/P1/deferred ordering for missing audit-log families, and the EF-Map request outline. The main confirmed live gap remains app-originated rename and power writes missing from `/activity`, while broader posture, policy, treasury/toll, and turret-doctrine history remains separate read-only backlog work.

> Status update (2026-05-05, merge and sponsor-worker activation): the sponsor-worker hardening pass is now merged to `master` at `1449dbb19a7113d8d4a19b9fab99ab35247d8e7b`, and the repo-owned worker was explicitly redeployed from clean `master` with Cloudflare version `0d0b68a4-5a79-4fea-acce-c8802f7cc43f`. Post-deploy worker checks passed: `npm run sponsor:validate-policy`, `npm run sponsor:test`, and `npm run sponsor:typecheck` all passed on the clean master worktree after installing the worker package lockfile dependencies, and `OPTIONS /sponsor` returned `204` with expected CORS headers for `https://civilizationcontrol.com`, `https://www.civilizationcontrol.com`, `https://civilizationcontrol.pages.dev`, and preview origin `https://cc2253d4.civilizationcontrol.pages.dev`. No Pages production deploy was performed; preview evidence continued on `https://cc2253d4.civilizationcontrol.pages.dev` and alias `https://fix-sponsor-policy-node-cont.civilizationcontrol.pages.dev`, where route smoke passed on `/`, `/nodes`, `/activity`, `/settings`, and `/dev/node-drilldown-lab`, and served-bundle scanning across all 12 JS assets found the repo sponsor URL plus `https://ef-map.com` and found no `flappy-frontier-sponsor`, `ASSEMBLY_API_TOKEN`, `X-API-Key`, `SPONSOR_PRIVATE_KEY`, `CF_API_TOKEN`, or `CLOUDFLARE_ACCOUNT_ID`. The later human smoke on `https://cc2253d4.civilizationcontrol.pages.dev` connected a wallet, performed a harmless structure rename, and confirmed the transaction was sponsored. That proves the activated sponsor worker avoided player-paid fallback for at least one current preview write path, and no further code, worker, Pages, EF-Map, VPS, Move, or package change was required.

> Status update (2026-05-05, sponsor-policy Node Control write pass): preview and wallet smoke had shown some current Node Control writes succeeding only through player-paid fallback after sponsor rejection. The committed allowlist was already aligned to the live Stillness write target surface on `master`: single structure power, generic attached-assembly power, rename, network-node online/offline teardown, mixed child-power PTBs, posture switching, gate-policy writes, extension authorization, and existing permit/trade flows were all present in `config/sponsorship/civilizationControlPolicy.ts` and mirrored in `workers/sponsor-service/wrangler.toml`. The real repo gap was validator correctness and proof depth, not a missing module/function list. `workers/sponsor-service/src/validation.ts` now rejects malformed owner-cap-gated world writes unless they match the current builder-shaped `borrow_owner_cap -> protected action(s) -> return_owner_cap` pattern, requires the full `network_node::offline -> return_owner_cap -> offline_connected_* -> destroy_offline_assemblies` teardown tail, and keeps the existing package/module/function allowlist, GasCoin blocking, command-kind filter, and command-count limits intact. Worker tests now cover accepted generic assembly power, rename, network-node online/offline teardown, mixed child-power, posture, gate-control, extension authorization plus metadata URL, and standalone market flows, and they reject missing borrow/return, malformed node-offline teardown, unknown package/module/function, command overflow, and arbitrary transfer-object mutation. `scripts/validate-sponsor-policy.mjs` now also checks `FUEL_CONFIG_ID` parity because node-offline sponsorship depends on that Stillness object. No EF-Map, VPS, Move, package-ID, vendor, or production deploy scope changed here, and no sponsor worker deploy was performed in this pass. Because the runtime worker was not redeployed, the most likely remaining live failure cause is stale deployed worker code or config rather than the committed repo policy.

> Status update (2026-05-05, local node power load truth): the remaining live Node Power defect after `f9190f5` was frontend model semantics, not a new write-path gap. Node Control row and bulk actions already updated selected-node child status locally through reconciliation overlays, but the meter still read `node.powerUsageSummary.usedGj` directly and the capacity guard still used indexed-summary-plus-delta math. The correction stays inside the accepted frontend boundary: `src/lib/nodePowerControlModel.ts` now derives current readout and final desired-state load from selected-node child rows when exact status plus `powerRequirement.requiredGj` are available, falls back to indexed summary only when row-local computation is unavailable, and therefore lets `Take all offline` resolve to `0 GJ` locally while preserving the existing one-PTB write path, fresh inventory preflight, direct-chain target guard, already-correct outcome summaries, hidden-child inclusion, and selected-node scope exclusions. Remaining native Node Power `title` tooltips were removed from `NodeStructureActionRail`, and the new live probe `scripts/check-node-power-live-usage.mts` now reports indexed summary versus local current/all-online load per live node with optional direct-chain child status snapshots via `--check-chain`. Validation passed across the widened node-power proof set, operator-inventory and selected-node probes, typecheck, explicit-env build, sponsor validation/tests/typecheck, Sui active env `testnet_stillness`, Move build/test, and `git diff --check`. Fresh preview proof is `https://bda884ea.civilizationcontrol.pages.dev` with alias `https://feat-network-node-offline-pr-3px2.civilizationcontrol.pages.dev`; unique and alias route smoke passed on `/`, `/nodes`, `/activity`, `/settings`, and `/dev/node-drilldown-lab`, the deployed lab stepped `Power 320 / 1000 GJ -> Power 200 / 1000 GJ -> Power 0 / 1000 GJ` through row-level `Take offline` actions, bring/take Node Power buttons had `title: null`, and served-bundle scanning across all 12 deployed JS assets found the repo sponsor URL plus `https://ef-map.com` and found no `flappy-frontier-sponsor`, exact-case `Authorization`, `ASSEMBLY_API_TOKEN`, or `X-API-Key`. No EF-Map contract, backend endpoint, Move/package-ID, sponsor-policy, vendor, production, broad-polling, browser `queryEvents`, or on-chain preset scope changed.

> Status update (2026-05-05, live node power summary mapping): human preview smoke showed the first indexed-power pass still rendered `Power usage unavailable` for a real wallet because the live grouped payload carries node usage on `networkNodes[].powerUsageSummary`, not `networkNodes[].node.powerUsageSummary`. A browser-origin fetch from the approved preview for wallet `0x11dd567e72d160ad7116a7358684dfff800af2a8e429cd1a65778640f8a61f62` returned `200`, 2 grouped nodes, and wrapper-level summaries only, with `usedGj` values `320` and `50` on the 11-child and 5-child nodes; child rows still carried `powerRequirement`. The frontend fix stays inside the accepted read boundary: the client normalizer now folds wrapper-level grouped summaries onto the node row, the adapter tolerates either the live wrapper-level shape or older nested payloads, `scripts/check-node-power-capacity.mts` now fails if only `node.powerUsageSummary` is read, and disabled Node Power strip controls no longer expose native `title` tooltips. Validation passed across the targeted node-power and operator-inventory probes, typecheck, build, sponsor policy/tests/typecheck, Sui active env `testnet_stillness`, and Move build/test. Fresh preview proof is `https://b2252cff.civilizationcontrol.pages.dev` with alias `https://feat-network-node-offline-pr-3px2.civilizationcontrol.pages.dev`; unique+alias route smoke passed on `/`, `/nodes`, `/activity`, `/settings`, and `/dev/node-drilldown-lab`, unique preview root smoke showed disabled `Commercial` and `Defensive` buttons with `title: null`, unique preview dev-lab smoke rendered `Node Drilldown Lab` and `Power 320 / 1000 GJ`, and served-bundle scanning across the deployed JS asset set found the repo sponsor URL plus `https://ef-map.com` while finding no `flappy-frontier-sponsor`, `ASSEMBLY_API_TOKEN`, or `X-API-Key`. The only remaining exact-case `Authorization` marker in the served bundle is user-facing `extension authorization` text, not a browser auth-header path.

> Status update (2026-05-05, indexed node power usage): the shipped Node Power State slice now consumes the live additive EF-Map/operator-inventory power fields without widening the accepted authority boundary. CivilizationControl threads per-child `powerRequirement` and node-level `powerUsageSummary` through the frontend model, renders a truthful `Power X / 1000 GJ` readout when the indexed summary plus child requirements are complete, counts hidden online children through the node summary, and blocks pre-wallet `Bring all online`, preset apply, and preset save when the requested or already-online child set would exceed the fixed 1000 GJ node budget. When indexed data is partial or missing, the UI stays calm and truthful with `Power usage unavailable` instead of fabricating a meter or capacity verdict. Deterministic proof now includes `scripts/check-node-power-capacity.mts` plus widened preset coverage; validation passed across typecheck, build, explicit-env build, the requested node-power and operator-inventory probes, sponsor policy/tests/typecheck, Sui active env `testnet_stillness`, and Move build/test. Fresh preview proof is `https://794ddc62.civilizationcontrol.pages.dev` with alias `https://feat-network-node-offline-pr.civilizationcontrol.pages.dev`; unique+alias route smoke passed on `/`, `/nodes`, `/activity`, `/settings`, and `/dev/node-drilldown-lab`; deployed browser smoke confirmed `Power 320 / 1000 GJ` in `Power Summary Known` and `Power usage unavailable` in `Power Summary Unknown`; and served-bundle scanning across the full unique preview JS set found the repo sponsor URL plus `https://ef-map.com` while finding no `flappy-frontier-sponsor`, header-like `Authorization` markers, `ASSEMBLY_API_TOKEN`, or `X-API-Key`. No EF-Map contract, VPS, Move contract, package-ID, vendor, production, broad polling, browser `queryEvents`, on-chain preset storage, or merge scope changed.

> Status update (2026-05-05, bulk outcome truth): the remaining Node Control problem after `c63dd38` was frontend truth projection, not missing runtime calls. Fresh operator-inventory and direct-chain preflight already knew which child rows were already correct, which rows survived to the mixed PTB, and which rows were held, but the old bulk path mostly discarded that decision set before local reconciliation and banner copy. Node Control now preserves those fresh decisions in one bulk outcome model, projects already-online/offline child rows into digest-null local overlays before wallet signing, and renders the existing `TxFeedbackBanner` with compact submitted/already-matched/held summaries plus row chips. `useStructurePower` also stops collapsing a mixed-batch correction into a misleading success when the remaining requested writes failed. The widened `scripts/check-operator-inventory-status-truth.mts` now supports `--wallet <address> --node <nodeObjectId> --action bring-online|take-offline` for live selected-node bulk classification. Validation passed across the targeted node-power proofs, `npm run typecheck`, and `npm run build`; fresh preview is `https://57e02fe6.civilizationcontrol.pages.dev` with alias `https://feat-network-node-offline-pr.civilizationcontrol.pages.dev`, unique+alias route smoke passed on `/`, `/nodes`, `/activity`, `/settings`, and `/dev/node-drilldown-lab`, unique preview browser smoke rendered `Node Drilldown Lab`, and served-bundle scanning across all 12 deployed JS assets found the repo sponsor URL, `https://ef-map.com`, and the new bulk outcome copy while finding no `flappy-frontier-sponsor`, exact-case `Authorization`, `ASSEMBLY_API_TOKEN`, or `X-API-Key`. No EF-Map, VPS, Move contract, package-ID, vendor, production, broad polling, browser `queryEvents`, on-chain preset storage, or merge scope changed.

> Status update (2026-05-05, final Node Power target guard): human wallet smoke after `59796a6` showed the previous Node Power runtime correction still allowed a stale offline child into `Take all offline`, so the PTB could fail during wallet resolution with `EAssemblyInvalidStatus` from `status::offline`. The abort-frame address `0xd2fd1224f881e7a705dbc211888af11655c315f2ee0f03fe680fc3176e6e4780` is the world runtime package/module address, not an operator-inventory child row; live row lookup returned `rawStatus: null`. The actual live stale rows for wallet `0x11dd567e72d160ad7116a7358684dfff800af2a8e429cd1a65778640f8a61f62` were refineries `0x5e7765cba30a546d092efc5494498be3668fe6ec33c6fd8b1dc5f93a156f0231` and `0xd2f58ab1724f4a93462ac4746b8221ed3d167e7f30ff4884bfb7aa2efdae7f70`, each EF-Map `rawStatus: online` but direct Sui `showContent` `status.variant: OFFLINE`. Node Control now refetches operator inventory, applies overlays, then runs a targeted direct-chain status preflight for only the remaining bulk/preset child targets and removes already-in-state children before `structurePower.toggleMixed(...)` builds the mixed PTB. Digest-null already-offline corrections are session-restored, standard wallet `FailedTransaction` preserves detailed errors where present, one mixed PTB remains for valid remaining targets, and an all-correct target set opens no wallet prompt. Deterministic proof covers stale raw-online direct-chain exclusion for bulk and preset apply, session correction persistence, one-PTB routing, hidden child inclusion, and other-node/unlinked/network-node-self exclusion. No EF-Map, VPS, Move contract, package-ID, vendor, on-chain preset storage, strategic posture preset, Signal Feed parity, marketplace, broad polling, browser `queryEvents`, production deploy, or merge scope changed.
> Validation and preview proof for the final target guard passed: typecheck, build, explicit-env build, diff-check, all requested Node Control/status probes, live wallet refinery classifier, sponsor policy/tests/typecheck, Sui active env `testnet_stillness`, Move build/test, feature preview deploy, unique+alias route smoke, served-bundle scan, and deployed-lab browser smoke. Fresh preview is `https://8dd2583a.civilizationcontrol.pages.dev` with alias `https://feat-network-node-offline-pr-3px2.civilizationcontrol.pages.dev`. EF-Map/operator-inventory status-indexing freshness is the required backend follow-up for the two raw-online/direct-chain-offline refinery rows.

> Status update (2026-05-05, Node Power online-target guard): human wallet smoke after `f57d309` showed the previous direct-chain guard only closed the offline-intent half of the problem. Single-row `Bring online` still bypassed final chain preflight, while bulk/preset online only removed exact chain `ONLINE` matches and otherwise failed open for neutral, missing, unexpected, or unavailable chain state. Node Control now preflight-refetches operator inventory, applies overlays, then uses targeted direct-chain snapshots for only the remaining candidate children and requires exact chain `OFFLINE` before online-intent targets can survive to wallet signing. Exact chain `ONLINE` evidence drops stale raw-offline online candidates before wallet prompt, non-`OFFLINE` online source states are excluded calmly, and an all-excluded online plan opens no wallet prompt. `status::online` is now classified conservatively: the app reports `Cannot bring online from current chain status.` unless a targeted post-failure chain recheck proves the target is already exact `ONLINE`, in which case CivilizationControl applies a digest-null correction and reports `Already online. View updated.` Session-restored digest-null power corrections now cover both already-offline and already-online evidence. Deterministic proof now covers bulk/preset online eligibility, single-row already-online correction, status-truth classification, and session persistence symmetry. Validation passed across the targeted Node Control proofs, typecheck, build, sponsor policy/tests/typecheck, Sui active env `testnet_stillness`, Move build/test, diff-check, and feature preview deploy. Fresh preview is `https://b6cb3d1d.civilizationcontrol.pages.dev` with alias `https://feat-network-node-offline-pr.civilizationcontrol.pages.dev`; unique+alias route smoke passed, served-bundle scanning across all 12 deployed JS assets found the repo sponsor URL, `https://ef-map.com`, `Cannot bring online from current chain status.`, and `Already online. View updated.`, and found no stale sponsor, exact-case `Authorization`, `ASSEMBLY_API_TOKEN`, or `X-API-Key`. No EF-Map, VPS, Move, package-ID, vendor, production, broad polling, browser `queryEvents`, on-chain preset storage, or merge scope changed.

> Status update (2026-05-05, local Node Power State runtime correction): human wallet smoke found that the local Node Power State UI needed runtime hardening after the first preview: child rows could render `Online` while chain/runtime already saw them offline, `status::offline` aborted with `EAssemblyInvalidStatus`, and bulk/preset apply submitted grouped approvals instead of one mixed child-power transaction. Node Control now preflight-refetches operator inventory, filters out already-in-target children, and submits all remaining gate/storage/turret/generic-assembly child writes through one mixed PTB via `structurePower.toggleMixed(...)`. `EAssemblyInvalidStatus` from `status::offline` is treated as already-offline evidence only for offline-intent writes, applies a digest-null offline correction overlay to the affected child, and cannot be immediately cleared by stale online operator-inventory data; the same abort remains an error path for online-intent writes. Deterministic proof now includes `scripts/check-node-power-batch-ptb.mts` and `scripts/check-operator-inventory-status-truth.mts`; the latter can live-inspect a supplied raw row with `--wallet <address> --object <objectId>`, but exact EF-Map row capture was skipped in this agent environment because no wallet address was available. Validation passed across typecheck, build, explicit-env build, all Node Control/status probes, sponsor policy/tests/typecheck, Sui active env, Move build/test, preview deploy, unique+alias route smoke, served-bundle scan, and deployed-lab browser smoke. Fresh preview proof is `https://6eb887e0.civilizationcontrol.pages.dev` with alias `https://feat-network-node-offline-pr-3px2.civilizationcontrol.pages.dev`; served-bundle scanning found the repo sponsor URL, `https://ef-map.com`, explicit Sui RPC, and `Already offline. View updated.`, and found no stale sponsor, exact-case `Authorization`, or checked secret/header markers. No EF-Map, VPS, Move contract, package-ID, on-chain preset storage, strategic posture preset, Signal Feed parity, marketplace, broad polling, browser `queryEvents`, production deploy, or merge scope changed.

> Status update (2026-05-04, local Node Power State controls): the previously deferred local Node Power State slice is now implemented on `feat/network-node-offline-proof` as browser-local/operator UX only. Node Control adds four local preset buttons, `Save power preset`, child-only `Take all offline`, child-only `Bring all online`, manual icon dragging, a `Reset layout` map control, and a calm `Power usage unavailable` readout. Bulk and preset apply affect only attached child structures for the selected network node, include hidden-from-map children, exclude the network-node anchor, exclude other nodes/unlinked structures, and pass per-child reconciliation targets for bounded overlay/refetch. Presets are localStorage/sessionStorage only under `cc:node-power-presets:v1:<scopeKey>:<nodeCanonicalId>`, scoped by character ID then wallet then session fallback, and store desired child online/offline state by canonical child identity. Manual layout is stored separately under `cc:node-drilldown:layout:v1:<scopeKey>:<nodeCanonicalId>`. Exact `used / 1000 GJ` metering and deterministic over-cap blocking are deferred because current frontend/operator-inventory data does not expose child `energyRequiredGJ` or node reserved/production energy fields; online bulk/preset operations require an app confirmation when capacity cannot be preverified, while chain execution remains final enforcement. Validation passed across typecheck, build, requested deterministic probes, new preset/layout probes, sponsor policy/tests/typecheck, and feature preview smoke. Fresh preview proof is `https://70c1585a.civilizationcontrol.pages.dev` with alias `https://feat-network-node-offline-pr.civilizationcontrol.pages.dev`; served-bundle scanning found the repo sponsor URL plus `https://ef-map.com` and no stale sponsor, exact-case `Authorization`, or checked secret/header markers. No EF-Map, VPS, Move, package-ID, on-chain preset storage, strategic posture preset, Signal Feed parity, marketplace, broad polling, browser `queryEvents`, production deploy, or merge scope changed.
