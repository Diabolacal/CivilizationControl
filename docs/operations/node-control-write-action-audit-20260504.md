...
> Status update (2026-05-04, network-node offline proof branch): the previously deferred node-offline seam is now proven and minimally implemented on `feat/network-node-offline-proof` without any package or Move change. Current Stillness runtime already exposes `world::network_node::offline`, but it returns `OfflineAssemblies` and therefore must be paired in the same PTB with the appropriate connected-child helper calls plus `network_node::destroy_offline_assemblies`; a bare single-call PTB is not sufficient. CivilizationControl now builds that PTB directly, using the live Stillness `FuelConfig` object `0x4fcf28a9be750d242bc5d2f324429e31176faecb5b84f0af7dff3a2a6e243550`, and exposes `Take offline` only on dedicated `/nodes` row menus and the network-node detail `Actions` surface once operator inventory or the `node-assemblies` fallback resolves all connected child identities. Sponsor policy now mirrors the full PTB shape under both world package IDs: `character::{borrow_owner_cap,return_owner_cap}`, `network_node::{offline,destroy_offline_assemblies}`, and the `assembly` / `gate` / `storage_unit` / `turret` `offline_connected_*` helpers. Validation passed: `scripts/check-network-node-offline-proof.mts`; `scripts/check-structure-action-support.mts`; `npm run sponsor:validate-policy`; `npm run sponsor:test`; `npm run sponsor:typecheck`; root typecheck/build; `sui client active-env`; `sui move build --path contracts/civilization_control`; and `sui move test --path contracts/civilization_control`. Preview proof is now `https://35907923.civilizationcontrol.pages.dev` with alias `https://feat-network-node-offline-pr.civilizationcontrol.pages.dev`; unique and alias `/`, `/nodes`, `/activity`, `/settings`, and `/dev/node-drilldown-lab` all resolved `/assets/index-iYv60cix.js`; served-bundle scanning found `https://civilizationcontrol-sponsor.michael-davis-home.workers.dev` plus `civilizationcontrol-sponsor` in `App-B_7wHB-z.js` and `SmartObjectProvider-krPfUEd5.js`, found `https://ef-map.com` in `nodeDrilldownMenuItems-Cl_6nR5I.js` and `SmartObjectProvider-krPfUEd5.js`, and found no `flappy-frontier-sponsor`, `ASSEMBLY_API_TOKEN`, exact-case `Authorization`, or `X-API-Key`. Node Control itself remains unchanged in this branch; the node-offline action is still intentionally scoped to dedicated network-node list/detail surfaces.
...
- dedicated network-node list/detail surfaces now support both `Bring online` and proof-gated `Take offline`; Node Control still does not expose node-level offline
...
> Phase 1A and Phase 1B are now both implemented without a package update: gate, storage, and turret power, network-node online, four-family rename, and generic attached-assembly power/rename now ride the existing zero-package runtime calls. Remaining deferred scope is network-node offline, extension freeze or revoke, marketplace settlement surfaces, and signal-history parity expansion.
...
| Individual `offline` | Network Node | Dedicated node list/detail surfaces now resolve connected-child identity from operator inventory or `node-assemblies` fallback before exposing the action | Shipped on dedicated `/nodes` row menus and node detail `Actions`; still not exposed in Node Control | Yes | `world::network_node::offline` plus same-PTB `offline_connected_*` handling and `destroy_offline_assemblies` | No | Implemented in the separate proof branch; keep Node Control node-level offline deferred |
...
|---|---|---|---|---|---|
| `networkNode` | Yes | `network_node` is in the current `StructureType` union | Partial; dedicated node list/detail surfaces now support proof-gated offline, but node-local rows still do not use this family | Network-node-specific power and metadata calls exist | Dedicated list/detail surfaces now support `online` plus proof-gated `offline`; keep Node Control node-level offline deferred |
