...
> Status update (2026-05-06, Node Control proof-loss repair): commit `4f5bc0e` and preview `https://ac0cd430.civilizationcontrol.pages.dev` still failed human wallet smoke, so that preview is regression evidence rather than proof of the fix. The failing screenshot shape was specific: selected-node rows carried display/canonical state, but Object ID, OwnerCap, Network Node, per-action authority, and power proof were missing, causing Node Control to hide row/node actions and show `Power usage unavailable` while global list actions still worked. The root cause is frontend proof loss across `src/lib/nodeDrilldownModel.ts` and `src/lib/operatorInventoryClient.ts`, not EF-Map/VPS, Move/package, sponsor, or Signal Feed drift. The selected-node `NodeLocalStructure` bucket merge could let weak display/fallback rows overwrite authority-bearing row fields, and `normalizeIndexedActionCandidate` failed to normalize live EF-Map top-level `actionCandidate.requiredIds` into per-action authority. The repair keeps operator inventory authoritative for object identity, owner-cap, network-node context, action candidates, and power data; fallback rows may only enrich presentation. `scripts/check-node-live-proof-loss.mts` plus `scripts/node-live-proof-loss-core.mts` now catch the screenshot condition directly. Fresh preview proof is `https://a187a67a.civilizationcontrol.pages.dev` with alias `https://fix-node-control-authority-r.civilizationcontrol.pages.dev`; both hosts route-smoked cleanly, the served entry asset is `/assets/index-C8BAInCf.js` rather than `index-Cwhp3oOh.js`, and the fresh-preview live diagnostic returned `proofLoss.ok: true` for the main node with Amazing Assembler retaining Object ID, OwnerCap, Network Node, verified `assembly` target, and `200 GJ` power requirement. No EF-Map/VPS contract, Move/package-ID, sponsor-worker, browser `queryEvents`, broad-polling, merge, production deploy, or package change was made.
