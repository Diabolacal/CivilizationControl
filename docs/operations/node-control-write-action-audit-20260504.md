...
> Status update (2026-05-06, rendered Node Control proof-loss repair): commit `ae80f9b` and preview `https://a187a67a.civilizationcontrol.pages.dev` still failed human wallet smoke after the adapted/model proof-loss scripts passed. The failure was in the rendered action target, not the backend or global action builders: weak canvas/list rows with assembly/display/status data could be selected or right-clicked, then the context menu and inspector/action surfaces reselected by render id or stale menu state before resolving to the authoritative operator-inventory row. The new shared resolver in `src/lib/nodeDrilldownSelection.ts` indexes selected-node rows by object ID, assembly ID, and canonical identity before render IDs, merges weak display rows into the strongest operator-inventory-backed row, and prevents fallback/node-assemblies presentation rows from erasing Object ID, OwnerCap, Network Node, action authority, or power proof. Canvas, attached-list rows, Selection Inspector, context menu, action rail, power readout, and child bulk planning now use the same resolved target. `scripts/check-node-live-proof-loss.mts` now proves the rendered seam with a live-shaped fixture where weak canvas/list input resolves to an authoritative row, the inspector exposes proof fields, the menu includes `Hide from Node View`, `Take Offline` or `Bring Online`, and `Rename Assembly`, the action rail is enabled, and power is not unavailable. `?debugNodeControl=1` exposes `window.__CC_NODE_CONTROL_DEBUG__.copySummary()` for human wallet sessions and is absent without the query parameter. Fresh feature preview proof is `https://a1aacf9b.civilizationcontrol.pages.dev` with alias `https://fix-node-control-authority-r.civilizationcontrol.pages.dev`; both hosts route-smoked cleanly, unique preview HTML served `/assets/index-D7u7M934.js` with app chunk `/assets/App-CJTkg-nE.js`, and served chunks contained `civilizationcontrol-sponsor` plus `https://ef-map.com` while finding no `flappy-frontier-sponsor`, `ASSEMBLY_API_TOKEN`, `X-API-Key`, `SPONSOR_PRIVATE_KEY`, `CF_API_TOKEN`, or `CLOUDFLARE_ACCOUNT_ID`. No EF-Map/VPS contract, Move/package-ID, sponsor-worker, browser `queryEvents`, broad-polling, WebSockets, merge, production deploy, or package change was made.
