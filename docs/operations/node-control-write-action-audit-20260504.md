...
> Status update (2026-05-04, corrective follow-up): human wallet-connected preview smoke on the first zero-package preview found one remaining parity gap and one read-freshness gap. Live Node Control backend-membership rows could still degrade to hide-only menus and locked rails when indexed power candidates omitted node context even though a unique live match already had the required IDs, and successful rename/power writes could snap back to stale operator-inventory values after one-shot invalidation. The corrective pass now resolves verified node-local targets from either indexed required IDs or a unique live match in `src/lib/nodeDrilldownModel.ts`, adds shared structure-write reconciliation overlays plus bounded retry across operator inventory, asset discovery, selected-node lookup, and active signal history, and proves both seams with `scripts/check-node-drilldown-reconciliation.mts` plus `scripts/check-structure-write-reconciliation.mts`. Fresh preview proof is now `https://d4129717.civilizationcontrol.pages.dev` with alias `https://feat-zero-package-action-par-da22.civilizationcontrol.pages.dev`; route smoke passed on `/`, `/nodes`, `/activity`, `/settings`, and `/dev/node-drilldown-lab`; dev-lab row-menu smoke still exposed `Hide from Node View`, `Take Offline`, and `Rename Assembly`; and served-bundle scanning found no `flappy-frontier-sponsor`, quoted `Authorization`, `ASSEMBLY_API_TOKEN`, `X-API-Key`, `SPONSOR_PRIVATE_KEY`, `CF_API_TOKEN`, or `CLOUDFLARE_ACCOUNT_ID` markers. The accepted zero-package boundary is unchanged: four-family rename, gate/storage/turret power, and network-node online only; network-node offline, presets, marketplace/revenue work, and signal-history parity expansion remain deferred.
