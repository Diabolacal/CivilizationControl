...
>
> Status update (2026-05-04, zero-package action parity): the next implementation wave is now live on preview at `https://2075be01.civilizationcontrol.pages.dev` and still respects the accepted read-path boundary. The new action slice adds shared power or rename menus plus explicit post-write refetch across operator inventory, asset discovery, selected-node `node-assemblies` fallback, and active signal history, but it does not change EF-Map contracts, does not restore browser `queryEvents` for normal Signal Feed routes, and does not widen shared-backend authority beyond additive read enrichment. Treat this as UI-level freshness on top of the current read model, not as a new read-path architecture change.
>
> Status update (2026-05-04, live Node Control corrective follow-up): human wallet smoke on the first zero-package preview proved the current read-path architecture itself was not the missing action surface, but it did expose one UI-level freshness gap inside the operator-inventory-first model. Successful rename/power writes could snap back to stale indexed values after one-shot invalidation, so the frontend now overlays pending write state and retries bounded refresh across operator inventory, asset discovery, selected-node `node-assemblies` fallback, and active signal history until the shared read model confirms. This does not widen backend authority, does not change EF-Map contracts, and does not reopen browser `queryEvents`; it is a temporary frontend reconciliation layer on top of the accepted read-path boundary. Fresh preview proof is now `https://d4129717.civilizationcontrol.pages.dev` with alias `https://feat-zero-package-action-par-da22.civilizationcontrol.pages.dev`, and the served bundle no longer carries the quoted `Authorization` header literal because the sponsor client now emits a lowercase header key while preserving request behavior.
>
