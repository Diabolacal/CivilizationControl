...
>
> Status update (2026-05-06, Node Control proof-loss repair): the post-merge Node Control regression still confirms the read-path boundary: operator inventory remains the authority source, and weaker fallback rows may only enrich presentation. Commit `4f5bc0e` and preview `https://ac0cd430.civilizationcontrol.pages.dev` still failed human wallet smoke; the failed state had selected-node display/canonical identity present while Object ID, OwnerCap, Network Node, action proof, and power proof were missing. The exact frontend seams were the selected-node `NodeLocalStructure` bucket merge in `src/lib/nodeDrilldownModel.ts`, which could let weak display/fallback rows erase authority-bearing fields, and `normalizeIndexedActionCandidate` in `src/lib/operatorInventoryClient.ts`, which did not normalize live EF-Map top-level `actionCandidate.requiredIds` into per-action authority. The repair does not change EF-Map endpoints, VPS behavior, browser `queryEvents`, Move/package state, sponsor-worker behavior, or production deploy scope. `scripts/check-node-live-proof-loss.mts` and `scripts/node-live-proof-loss-core.mts` now fail on the screenshot condition directly, so future fallback/display enrichment cannot silently strip operator-inventory object, owner-cap, network-node, action, or power proof. Fresh feature-preview proof is `https://a187a67a.civilizationcontrol.pages.dev` with alias `https://fix-node-control-authority-r.civilizationcontrol.pages.dev`; both hosts returned `200` for the standard smoke routes, the unique host served `/assets/index-C8BAInCf.js`, and the fresh-preview origin live diagnostic returned `proofLoss.ok: true` for the main node.
>
