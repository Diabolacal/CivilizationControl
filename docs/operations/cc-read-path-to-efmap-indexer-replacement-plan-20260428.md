...
>
> Status update (2026-05-06, rendered Node Control proof-loss repair): the `ae80f9b` preview at `https://a187a67a.civilizationcontrol.pages.dev` proved model-level proof was not sufficient. Operator inventory could contain the complete row while the rendered Node Control path still used a weak canvas/list/context-menu projection that had assembly/display/status data but no Object ID, OwnerCap, Network Node context, action authority, or power proof. The corrected boundary is now explicit: operator inventory remains the authority source, fallback/node-assemblies rows may enrich presentation only, and all rendered action surfaces must resolve weak selections through the selected-node authority map before showing proof or enabling writes. `src/lib/nodeDrilldownSelection.ts` keys that map by object ID, assembly ID, and canonical identity before render IDs; `Dashboard`, the canvas, attached list, Selection Inspector, context-menu builder, action rail, power readout, and child bulk planning now share that resolver. The diagnostic no longer stops at the adapted model: `scripts/check-node-live-proof-loss.mts` renders/live-shapes a weak selected target, proves the resolved inspector/action rail/context menu target has proof, and fails if power would render as unavailable while selected-node power summary plus child requirements are present. `?debugNodeControl=1` adds a browser-only `window.__CC_NODE_CONTROL_DEBUG__.copySummary()` for human wallet sessions and is removed when the query param is absent. Fresh feature-preview proof is `https://a1aacf9b.civilizationcontrol.pages.dev` with alias `https://fix-node-control-authority-r.civilizationcontrol.pages.dev`; both hosts returned `200` for the standard smoke routes, the unique host served `/assets/index-D7u7M934.js` with app chunk `/assets/App-CJTkg-nE.js`, and served chunks contained `civilizationcontrol-sponsor` plus `https://ef-map.com` while finding no checked stale sponsor or secret markers. No EF-Map endpoint, VPS behavior, browser `queryEvents`, broad polling, WebSockets, Move/package state, sponsor-worker behavior, merge, or production deploy scope changed.
>
