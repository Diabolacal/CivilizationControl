...
- After human wallet-connected preview smoke, the macro Strategic Network strip is intentionally quieter: the helper narration under `Commercial` / `Defensive` was removed, and the placeholder `Save Preset` affordance was removed rather than being migrated into this branch. Node Power State presets still belong later in Node Control, not in the Strategic Network macro strip.
- Human smoke also confirmed that turret-only or no-gate posture switching can succeed after wallet and live world state settle, but stale post-write status or read-model refresh after in-app power actions remains a separate zero-package write-action parity follow-up and was not widened here.
