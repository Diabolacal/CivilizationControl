...
- Commit `4f5bc0e` and preview `https://ac0cd430.civilizationcontrol.pages.dev` still failed human wallet smoke. The screenshot condition was a selected-node row with display/canonical state present, but Object ID, OwnerCap, Network Node, action proof, and power proof missing, so Node Control hid write actions and fell back to `Power usage unavailable`.
- Root cause is frontend proof loss across two seams, not EF-Map/VPS, Move/package, or sponsor drift. In `src/lib/nodeDrilldownModel.ts`, the selected-node `NodeLocalStructure` bucket merge could let weak display/fallback rows carry canonical/display state while dropping the stronger row's `objectId`, `ownerCapId`, network-node context, action candidate, and power proof. In `src/lib/operatorInventoryClient.ts`, `normalizeIndexedActionCandidate` did not normalize live EF-Map top-level `actionCandidate.requiredIds` into per-action authority, so indexed rows could appear present without usable action proof.
- Operator inventory remains the selected-node authority source for object identity, owner-cap, network-node, action, and power data. Fallback rows may enrich presentation only: names, display status, icons, and similar UI fields must not erase authority-bearing or power-bearing fields from stronger operator-inventory rows.
- The live proof-loss diagnostic now guards the actual screenshot shape. `scripts/check-node-live-proof-loss.mts` and `scripts/node-live-proof-loss-core.mts` fail when a selected-node row has display/canonical identifiers but lacks Object ID, OwnerCap, Network Node, per-action authority, or power proof.
- Fresh preview proof is `https://a187a67a.civilizationcontrol.pages.dev` with alias `https://fix-node-control-authority-r.civilizationcontrol.pages.dev`. Unique and alias route smoke returned `200` for `/`, `/activity`, `/nodes`, `/settings`, and `/dev/node-drilldown-lab`; the unique host served `/assets/index-C8BAInCf.js`, not the failed `index-Cwhp3oOh.js`; and the fresh-preview live diagnostic for the main wallet/node returned `proofLoss.ok: true` with Amazing Assembler retaining Object ID, OwnerCap, Network Node, verified `assembly` target, and `200 GJ` power requirement.
- Scope remains narrow: no EF-Map or VPS change, no Move/package-ID change, no sponsor-worker change, no merge, and no production deploy. The failed `ac0cd430` preview is retained as regression evidence, not as proof of the fix.
- Remaining known gaps stay separate: historical rename rows are not backfilled into current Signal Feed proof, fresh CivilizationControl custom-package rows still need live action proof when new writes exist, gate-specific live smoke remains weak until real gate rows exist on the test wallet, and external marketplace coverage is still deferred.
