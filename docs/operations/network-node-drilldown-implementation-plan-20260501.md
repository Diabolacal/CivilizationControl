...
- Commit `ae80f9b` and preview `https://a187a67a.civilizationcontrol.pages.dev` still failed human wallet smoke. The model/live diagnostic had complete proof, but the rendered UI still selected a weak child structure projection: Selection Inspector showed missing Object ID, OwnerCap, and Network Node, the right-click menu only showed `Hide from Node View`, row actions stayed disabled, and the meter fell back to `Power usage unavailable`.
- The confirmed root cause is the rendered selected-row/action-target seam. Canvas/list/context menu selections may start from weak display rows with assembly ID, label, and status only. Before this repair, those weak rows could be reselected by render id or stale menu `nextOnline` and used by the inspector/menu/action rail instead of the authoritative selected-node operator-inventory row. The previous `a187a67a` proof therefore validated the adapted model, not the object React rendered and used for actions.
- Final human-smoked resolution on `master` confirmed the deeper selected-node lookup failure: the operator-inventory lookup for the selected node could time out or miss the authoritative row, letting the weaker `node-assemblies` fallback become the rendered action source. The final fix hardened and lengthened the selected-node operator-inventory lookup path so fallback data can still enrich display state but no longer wins over object ID, OwnerCap ID, Network Node ID, rename proof, power proof, or current load authority.
- `src/lib/nodeDrilldownSelection.ts` now provides one resolver for rendered Node Control surfaces. It builds the selected-node authority map by object ID, assembly ID, and canonical identity before render IDs, scores rows by proof strength, and merges weak canvas/list/menu selections into the strongest operator-inventory-backed row. Fallback and node-assemblies rows may enrich names, display status, and icons, but they may not erase object, owner-cap, network-node, action, or power proof.
- Dashboard, `NodeDrilldownCanvas`, `NodeStructureListPanel`, `NodeSelectionInspector`, the context-menu builder, the action rail target, node power readout inputs, and child bulk planning now use the same resolved row before rendering proof or enabling actions. Map icon right-click and attached-list row right-click therefore build from the same resolved authority target.
- The browser-only debug seam is `?debugNodeControl=1`. It publishes `window.__CC_NODE_CONTROL_DEBUG__.copySummary()` with selected node/structure identity, object/assembly/owner-cap/network-node/energy-source IDs, provenance, action authority, `canRename`, `canBringOnline`, `canTakeOffline`, context-menu labels, action rail state, bulk controls, power readout inputs, and raw/adapted proof. The debug object is absent without the query parameter.
- The deterministic and live diagnostics now cover the rendered target. `scripts/check-node-live-proof-loss.mts --fixture-only` uses a live-shaped weak canvas/list row and an authoritative operator-inventory row for the same assembly ID, then asserts that the rendered inspector shows Object ID/OwnerCap/Network Node proof, the menu includes `Hide from Node View`, `Take Offline` or `Bring Online`, and `Rename Assembly`, the action rail is enabled, and power is not unavailable. The live run also reports the weak input, resolved inspector target, action rail target, and context-menu target.
- Validation passed for the requested frontend/read-model gates. `check-node-power-live-usage` hit one transient `502 Bad Gateway` from EF-Map on the first run, then passed on retry with the main node at `Power 770 / 1000 GJ`.
- Fresh preview proof is `https://a1aacf9b.civilizationcontrol.pages.dev` with alias `https://fix-node-control-authority-r.civilizationcontrol.pages.dev`. Unique and alias route smoke returned `200` for `/`, `/activity`, `/nodes`, `/settings`, and `/dev/node-drilldown-lab`; browser smoke proved the debug export is absent without `?debugNodeControl=1` and present with `copySummary()` when enabled; the unique host served `/assets/index-D7u7M934.js` with app chunk `/assets/App-CJTkg-nE.js`; and served JS chunks contained `civilizationcontrol-sponsor` plus `https://ef-map.com` while finding no `flappy-frontier-sponsor`, `ASSEMBLY_API_TOKEN`, `X-API-Key`, `SPONSOR_PRIVATE_KEY`, `CF_API_TOKEN`, or `CLOUDFLARE_ACCOUNT_ID`.
- Scope remains narrow: no EF-Map or VPS change, no Move/package-ID change, no sponsor-worker change, no merge, and no production deploy. The failed `a187a67a` preview is retained as regression evidence, not as proof of the fix.
- Remaining known gaps stay separate: the agent cannot perform a wallet-connected smoke in this environment, historical rename rows are not backfilled into current Signal Feed proof, fresh CivilizationControl custom-package rows still need live action proof when new writes exist, gate-specific live smoke remains weak until real gate rows exist on the test wallet, and external marketplace coverage is still deferred.
...
- Human preview smoke after `f9190f5` showed the remaining defect was model semantics, not EF-Map shape: the UI could render the live starting readout (`Power 320 / 1000 GJ`), but row and bulk actions still kept the meter pinned to indexed `usedGj` and could still evaluate capacity from stale summary-plus-delta math.
- `src/lib/nodePowerControlModel.ts` now computes current readout and final desired-state load from the selected node child rows whenever exact row status plus `powerRequirement.requiredGj` are available. Indexed node summary remains fallback metadata only when local row computation is unavailable, so `Take all offline` can now resolve to `0 GJ` locally and row actions update the meter immediately.
- Hidden selected-node children still count because the full selected-node structure list feeds the model, while other-node, unlinked, and network-node-self rows remain excluded by the selected-node view-model boundary.
- Remaining native `title` tooltips are removed from `NodeStructureActionRail`, so deployed Node Power bring/take controls no longer rely on browser tooltips.
- New live diagnostic `scripts/check-node-power-live-usage.mts` fetches operator inventory from a browser origin, adapts selected nodes, reports indexed summary versus local current and all-online load, and can optionally batch direct-chain child status snapshots with `--check-chain`. Live runs for wallet `0x11dd567e72d160ad7116a7358684dfff800af2a8e429cd1a65778640f8a61f62` reported two grouped nodes with indexed/local load agreement (`320/770` and `50/500`); the chain-checked first node exposed stale indexed-versus-chain status on refineries `0x5e7765cba30a546d092efc5494498be3668fe6ec33c6fd8b1dc5f93a156f0231` and `0xd2f58ab1724f4a93462ac4746b8221ed3d167e7f30ff4884bfb7aa2efdae7f70` plus several other child rows.
- Deterministic proof now explicitly covers local all-offline `0 / 1000 GJ`, a local `+50 GJ` bring-online increment, hidden-child inclusion, selected-node exclusion of node-self or other-node or unlinked rows, under-cap `770 GJ` all-online allowance, over-cap `1120 GJ` blocking, Take all offline never blocking for capacity, preset save/apply final-state evaluation, one-PTB preservation, and no native `title` tooltips in Node Power controls.
- Validation refreshed: `node --import tsx scripts/check-node-power-capacity.mts`; `node --import tsx scripts/check-node-power-presets.mts`; `node --import tsx scripts/check-node-power-batch-ptb.mts`; `node --import tsx scripts/check-node-layout-overrides.mts`; `node --import tsx scripts/check-operator-inventory-mapping.mts`; `node --import tsx scripts/check-node-drilldown-reconciliation.mts`; `node --import tsx scripts/check-operator-inventory-status-truth.mts`; `node --import tsx scripts/check-structure-write-reconciliation.mts`; `node --import tsx scripts/check-node-control-action-projection.mts`; `node --import tsx scripts/check-node-power-live-usage.mts --wallet 0x11dd567e72d160ad7116a7358684dfff800af2a8e429cd1a65778640f8a61f62`; `node --import tsx scripts/check-node-power-live-usage.mts --wallet 0x11dd567e72d160ad7116a7358684dfff800af2a8e429cd1a65778640f8a61f62 --node 0x2deb4248ed82ecbe42410e6ff4f8902f2e48b0c348c3dfdfb3f2c83acde73b85 --check-chain`; `npm run typecheck`; explicit-env `npm run build`; `npm run sponsor:validate-policy`; `npm run sponsor:test`; `npm run sponsor:typecheck`; `sui client active-env`; `sui move build --path contracts/civilization_control`; `sui move test --path contracts/civilization_control`; and `git diff --check`. Move build/test only emitted the existing `gate_control.move` non-composable self-transfer lint warning.
- Fresh preview proof is `https://bda884ea.civilizationcontrol.pages.dev` with alias `https://feat-network-node-offline-pr-3px2.civilizationcontrol.pages.dev`; unique and alias `/`, `/nodes`, `/activity`, `/settings`, and `/dev/node-drilldown-lab` returned `200` and both hosts served `/assets/index-G702q-VZ.js`; unique preview lab smoke loaded `Power Summary Known`, rendered `Power 320 / 1000 GJ`, then stepped to `Power 200 / 1000 GJ` after `Storage Alpha` `Take offline` and to `Power 0 / 1000 GJ` after `Hidden Gate Beta` `Take offline`; deployed Node Power bring/take buttons had `title: null`; served-bundle scanning across all 12 deployed JS assets found the repo sponsor URL in `App-BwlBvqY_.js` and `SmartObjectProvider-DDbFOhio.js`, found `https://ef-map.com` and `Power usage unavailable` in `nodePowerControlModel-BD3L3H8C.js`, and found no `flappy-frontier-sponsor`, exact-case `Authorization`, `ASSEMBLY_API_TOKEN`, or `X-API-Key`.
...
- Human wallet smoke after `f57d309` proved the previous direct-chain guard was still asymmetric: single-row `Bring online` bypassed final chain preflight entirely, and bulk/preset online filtering only removed exact chain `ONLINE` matches while failing open on neutral, missing, unexpected, or unavailable chain state.
- Node Control now treats online-intent eligibility as a proof boundary. After operator-inventory refetch and overlay filtering, both bulk/preset execution and single-row `Bring online` run a targeted direct-chain `multiGetObjects({ showContent: true })` snapshot read for only the remaining candidate children and require exact chain `OFFLINE` before an online target can survive to wallet signing.
- Exact chain `ONLINE` evidence now drops stale raw-offline online candidates before wallet prompt, and non-`OFFLINE` online source states are excluded calmly instead of reaching `status::online`. If every online candidate is excluded, the action becomes a no-op and no wallet prompt opens.
- `status::online` is no longer treated like `status::offline` without proof. Wallet-resolution `status::online` failures now classify as `Cannot bring online from current chain status.` unless a targeted post-failure chain recheck proves the target is already exact `ONLINE`, in which case CivilizationControl applies a digest-null local correction and reports `Already online. View updated.`
- Digest-null power corrections are now session-restored for both already-offline and already-online evidence, so stale operator-inventory rows cannot immediately re-poison repeat online/offline plans inside the current browser session.
- Deterministic proof now covers direct-chain `ONLINE` exclusion for `Bring all online`, direct-chain `OFFLINE` inclusion for valid online targets, neutral/unavailable online-source rejection, preset-online symmetry, single-row already-online correction, mixed-PTB routing, status-truth classification, and session persistence for both digest-null offline and online corrections.
- Validation refreshed: `npx tsx scripts/check-node-power-presets.mts`; `npx tsx scripts/check-node-power-batch-ptb.mts`; `npx tsx scripts/check-operator-inventory-status-truth.mts`; `npx tsx scripts/check-structure-write-reconciliation.mts`; `npx tsx scripts/check-network-node-offline-proof.mts`; `npm run typecheck`; `npm run build`; `npm run sponsor:validate-policy`; `npm run sponsor:test`; `npm run sponsor:typecheck`; `sui client active-env` (`testnet_stillness`); `sui move build --path contracts/civilization_control`; `sui move test --path contracts/civilization_control`; and `git diff --check` with only the preserved unrelated `contracts/civilization_control/Move.lock` LF/CRLF warning. Move build/test only emitted the existing `gate_control.move` non-composable self-transfer lint warning.
- Fresh preview proof is `https://b6cb3d1d.civilizationcontrol.pages.dev` with alias `https://feat-network-node-offline-pr.civilizationcontrol.pages.dev`; unique and alias `/`, `/nodes`, `/activity`, `/settings`, and `/dev/node-drilldown-lab` returned `200`, both hosts served `/assets/index-DVEqkkFS.js`, served-bundle scanning across all 12 deployed JS assets found the repo sponsor URL, `https://ef-map.com`, `Cannot bring online from current chain status.`, and `Already online. View updated.`, and found no `flappy-frontier-sponsor`, exact-case `Authorization`, `ASSEMBLY_API_TOKEN`, or `X-API-Key`. Deployed browser smoke confirmed the lab route loads the full synthetic Node Control surface.

### Node Power final-target guard - 2026-05-05

This corrective pass stays on `feat/network-node-offline-proof`. It does not change EF-Map contracts, VPS state, Move contracts, package IDs, vendor state, production deploy state, on-chain preset storage, Signal Feed parity, marketplace/revenue behavior, browser `queryEvents`, or broad polling.

- Human wallet smoke after `59796a6` proved the earlier runtime correction was still insufficient: `Take all offline` could still send a child that the chain already considered offline, and wallet resolution rejected the PTB before post-submit reconciliation could help.
- The supplied abort-frame address `0xd2fd1224f881e7a705dbc211888af11655c315f2ee0f03fe680fc3176e6e4780` is the world runtime package address in `0xd2fd...::status::offline`, not an operator-inventory child row. Live operator-inventory lookup for that address returned `rawStatus: null`.
- Browser-origin live probes for wallet `0x11dd567e72d160ad7116a7358684dfff800af2a8e429cd1a65778640f8a61f62` found the stale child rows: refineries `0x5e7765cba30a546d092efc5494498be3668fe6ec33c6fd8b1dc5f93a156f0231` and `0xd2f58ab1724f4a93462ac4746b8221ed3d167e7f30ff4884bfb7aa2efdae7f70` had EF-Map `rawStatus: online` while direct Sui `showContent` reported `status.fields.status.variant: OFFLINE` for both.
- Node Control now treats the pre-wallet target list as the proof boundary. After operator-inventory refetch and overlay filtering, bulk/preset execution runs a targeted direct-chain `multiGetObjects({ showContent: true })` for only the remaining candidate children and drops any target whose chain `ONLINE` / `OFFLINE` variant already matches the requested state before calling `buildMixedAssemblyPowerTx(...)`.
- Digest-null already-offline correction overlays are also restored from session storage, so a wallet-resolution `EAssemblyInvalidStatus` correction survives the current browser session and stale online operator-inventory rows cannot re-poison a repeat offline bulk/preset plan. Standard wallet `FailedTransaction` handling now preserves detailed failure text when available, so `status::offline` aborts remain classifiable.
- One-PTB behavior remains intact for the surviving target set: Node Control still calls `structurePower.toggleMixed(...)` once and only removes already-correct children before that call. If every child is already in the requested state, the action becomes a no-op and no wallet prompt opens.
- Deterministic proof now covers stale raw-online plus direct-chain offline exclusion for both `Take all offline` and offline preset apply, session-restored already-offline corrections, final mixed PTB routing, hidden child inclusion, other-node/unlinked/network-node-self exclusion, and the absence of new Node Control `queryEvents` usage.
- Validation refreshed: `npm run typecheck`; `npm run build`; `git diff --check` with only the preserved unrelated `contracts/civilization_control/Move.lock` LF/CRLF warning; all Node Control/status probes; sponsor policy/tests/typecheck; `sui client active-env` (`testnet_stillness`); `sui move build --path contracts/civilization_control`; and `sui move test --path contracts/civilization_control`. Move build/test only emitted the existing `gate_control.move` non-composable self-transfer lint warning.
- Durable follow-up: EF-Map/operator-inventory status indexing should stop serving stale `online` for those refinery rows. The frontend guard is intentionally narrow and action-scoped; it does not introduce a new broad polling or browser event-query read path.

### Local Node Power State controls - 2026-05-04

This implementation pass stays on `feat/network-node-offline-proof` and adds Node Control local/operator UX only. It does not change EF-Map, VPS state, Move contracts, package IDs, vendor state, production deploy state, Signal Feed parity, marketplace/revenue behavior, browser `queryEvents`, or the strategic macro posture/preset model.

- Same-day live-shape correction: human preview smoke showed the first indexed-power pass still rendered `Power usage unavailable` for a real wallet because the live grouped payload exposes node usage on `networkNodes[].powerUsageSummary`, not `networkNodes[].node.powerUsageSummary`. A browser-origin fetch from the approved preview for wallet `0x11dd567e72d160ad7116a7358684dfff800af2a8e429cd1a65778640f8a61f62` returned `200`, 2 grouped nodes, and wrapper-level summaries only, with `usedGj` values `320` and `50` on the 11-child and 5-child nodes. Child rows still carried `structures[].powerRequirement`.
- `normalizeOperatorInventoryResponse(...)` now folds wrapper-level grouped summaries onto the node row while retaining the live grouped field, and the operator-inventory adapter tolerates either the live wrapper-level shape or older nested `node.powerUsageSummary` payloads so selected Node Control can keep the same node-local readout path.
- `scripts/check-node-power-capacity.mts` now uses that live wrapper-level shape, so the deterministic probe fails if code only reads `node.powerUsageSummary`. `NodePowerActionStrip` also no longer assigns native browser `title` tooltips to disabled preset or bulk controls.
- Fresh preview proof for the corrective pass is `https://b2252cff.civilizationcontrol.pages.dev` with alias `https://feat-network-node-offline-pr-3px2.civilizationcontrol.pages.dev`; unique and alias route smoke passed on `/`, `/nodes`, `/activity`, `/settings`, and `/dev/node-drilldown-lab`, both hosts served `/assets/index-D6cyWpRQ.js`, unique root browser smoke showed disabled `Commercial` and `Defensive` buttons with `title: null`, unique dev-lab smoke rendered `Node Drilldown Lab` plus `Power 320 / 1000 GJ`, and served-bundle scanning across the deployed JS asset set found the repo sponsor URL plus `https://ef-map.com` while finding no `flappy-frontier-sponsor`, `ASSEMBLY_API_TOKEN`, or `X-API-Key`. The only remaining exact-case `Authorization` string is user-facing `extension authorization` text, not a browser auth-header path.

- Node Control now has a grouped top-right command strip in the existing panel header: four local preset buttons, `Save power preset`, `Take all offline`, `Bring all online`, and `Back to Strategic Network` on the far right.
- `Take all offline` and `Bring all online` target only child structures attached to the selected network node. They filter by the current node-local child membership, include hidden-from-map children, ignore children already in the requested state, exclude structures from other nodes or unlinked structures, and never include the network-node anchor itself.
- Child bulk and preset apply reuse the existing per-child power write path through grouped `toggleBatch` calls by structure action target type. The implementation passes per-child reconciliation targets so local overlays apply immediately and bounded operator-inventory / selected-node refresh can clear only after the read model confirms the target state.
- Four local power preset slots are implemented per network node. Presets are browser-local only and stored under `cc:node-power-presets:v1:<scopeKey>:<nodeCanonicalId>` or `cc:node-power-presets:v1:session:<nodeCanonicalId>`, where `scopeKey` continues to prefer character ID, then wallet address, then session fallback. Presets save desired child online/offline state by stable canonical child identity; they do not store icon positions and do not alter the network node, strategic posture, gate commercial/defensive posture presets, or any on-chain preset object.
- Preset apply computes a diff against current child state and writes only changed child structures. Missing old structures are ignored, new structures absent from the preset are left unchanged, and all-missing or no-change slots fail calmly through disabled/no-op state. While a write is pending, repeat bulk/preset submissions are locked out by the existing transaction state.
- Manual node-local icon layout is implemented as local logical percentage coordinates, stored separately under `cc:node-drilldown:layout:v1:<scopeKey>:<nodeCanonicalId>` or `cc:node-drilldown:layout:v1:session:<nodeCanonicalId>`. Dragging a map icon persists a clamped logical position for that child identity; hidden rows keep their stored position while hidden and reappear there when unhidden.
- `Reset layout` appears in the Node Control map-local control strip beside `hide key`. It clears only manual layout overrides for the current node and does not clear hidden/unhidden state, power presets, or online/offline state. No wallet, transaction, or confirmation is required.
- Node Control now consumes the live additive operator-inventory power fields: per-child `powerRequirement` and node-level `powerUsageSummary`. When the node summary is complete and the current or requested online child set has trustworthy requirements, the map strip renders a truthful `Power X / 1000 GJ` readout instead of fabricated math. Hidden online children still count through the indexed node summary.
- Capacity evaluation now uses the fixed 1000 GJ node rule with indexed child requirements. Pre-wallet `Bring all online`, preset apply, and preset save block locally when the requested or already-online child set would exceed 1000 GJ. `Take all offline` still bypasses capacity checks. If the node summary is partial or any required child power field is missing, the UI falls back calmly to `Power usage unavailable` and does not fabricate a capacity verdict.
- Follow-up for remaining power-model refinement: keep current indexed-field truth as the frontend authority. A later EF-Map/operator-inventory enrichment pass can still expose richer source or freshness metadata and energy-source production or reserve fields, but the frontend should not infer power from names or hardcode family values.
- Deterministic proof now includes `scripts/check-node-power-capacity.mts`, `scripts/check-node-power-presets.mts`, and `scripts/check-node-layout-overrides.mts`. Coverage now includes numeric-versus-unavailable readout behavior, hidden-online-child inclusion, over-cap bring-online blocking, preset-save blocking for already-over-cap online sets, unavailable-state false-confidence blocking, scoped storage keys, local overlay target propagation, layout coordinate persistence, clamping, hidden or unhidden layout preservation, and reset-layout scope.
- Validation passed: `npm run typecheck`; `npm run build`; explicit-env preview `npm run build` with `VITE_SPONSOR_URL=https://civilizationcontrol-sponsor.michael-davis-home.workers.dev`, `VITE_SHARED_BACKEND_URL=https://ef-map.com`, blank `VITE_SPONSOR_API_KEY`, and `VITE_SUI_RPC_URL=https://fullnode.testnet.sui.io:443`; `git diff --check` with only the preserved unrelated `contracts/civilization_control/Move.lock` LF/CRLF warning; `npx tsx scripts/check-node-power-presets.mts`; `npx tsx scripts/check-node-power-batch-ptb.mts`; `npx tsx scripts/check-node-power-capacity.mts`; `npx tsx scripts/check-operator-inventory-mapping.mts`; `npx tsx scripts/check-node-drilldown-reconciliation.mts`; `npx tsx scripts/check-operator-inventory-status-truth.mts`; `npm run sponsor:validate-policy`; `npm run sponsor:test`; `npm run sponsor:typecheck`; `sui client active-env` (`testnet_stillness`); `sui move build --path contracts/civilization_control`; and `sui move test --path contracts/civilization_control`. Move build/test only emitted the existing `gate_control.move` non-composable self-transfer lint warning.
- Fresh preview proof is `https://794ddc62.civilizationcontrol.pages.dev` with alias `https://feat-network-node-offline-pr.civilizationcontrol.pages.dev`; unique and alias route smoke passed on `/`, `/nodes`, `/activity`, `/settings`, and `/dev/node-drilldown-lab`, and both hosts served `/assets/index-BxJXXh-2.js`. Deployed dev-lab browser smoke confirmed `Power Summary Known` renders `Power 320 / 1000 GJ`, `Power Summary Unknown` renders `Power usage unavailable`, and the isolated synthetic route still avoids wallet, sponsor, shared-backend, and transaction flows. Served-bundle scanning across the unique preview JS assets found the repo sponsor URL, `https://ef-map.com`, and `Power usage unavailable`, and found no `flappy-frontier-sponsor`, header-like `Authorization` markers, `ASSEMBLY_API_TOKEN`, `X-API-Key`, `SPONSOR_PRIVATE_KEY`, `CF_API_TOKEN`, or `CLOUDFLARE_ACCOUNT_ID`. Wallet smoke remained unavailable in the agent browser because no usable wallet-backed session was present.

### Final Node Power target guard - 2026-05-05

This corrective pass stays on `feat/network-node-offline-proof`. It does not change EF-Map contracts, VPS state, Move contracts, package IDs, vendor state, production deploy state, on-chain preset storage, Signal Feed parity, marketplace/revenue behavior, browser `queryEvents`, broad polling, or UI layout scope.

- Human wallet smoke after `59796a6` proved the previous post-error correction was insufficient: stale raw-online children could still reach `buildMixedAssemblyPowerTx(...)` and wallet transaction resolution could abort before any successful write with `EAssemblyInvalidStatus` from `status::offline`.
- Live classification for wallet `0x11dd567e72d160ad7116a7358684dfff800af2a8e429cd1a65778640f8a61f62` showed the supplied abort-frame address `0xd2fd1224f881e7a705dbc211888af11655c315f2ee0f03fe680fc3176e6e4780` is absent from operator-inventory rows and has no object status; it is the world runtime package/module address in the Move frame. The actual stale rows were grouped refinery children `0x5e7765cba30a546d092efc5494498be3668fe6ec33c6fd8b1dc5f93a156f0231` and `0xd2f58ab1724f4a93462ac4746b8221ed3d167e7f30ff4884bfb7aa2efdae7f70`, both EF-Map `rawStatus: online`, `source: shared-frontier-backend`, `provenance: operator-inventory-indexer`, and direct Sui `status.variant: OFFLINE`.
- Node Control now refetches operator inventory, applies local/session overlays, then performs a targeted direct-chain `multiGetObjects(showContent)` read for only the final bulk/preset child candidates. `filterNodePowerPlanForTargetStatuses(...)` removes children whose direct-chain status already matches the requested target state before `structurePower.toggleMixed(...)` builds the one mixed PTB. If all candidates are removed, no wallet prompt opens.
- Digest-null already-offline corrections from wallet-resolution failures are persisted in session storage and restored into reconciliation overlays, so stale online operator-inventory rows cannot immediately reintroduce the same target in the current browser session. Standard wallet `FailedTransaction` details are preserved for classifier input, mapped offline `EAssemblyInvalidStatus` records a correction, unmapped multi-target aborts do not mark unrelated children complete, and unrelated Move aborts still surface as failures.
- Deterministic proof now covers stale raw-online direct-chain-offline exclusion for `Take all offline` and offline preset apply, final mixed-PTB command shape for valid remaining targets, hidden child inclusion, other-node/unlinked/network-node-self exclusion, session correction persistence, direct-chain parser behavior, wallet-resolution error mapping, online-intent non-swallowing, unrelated MoveAbort surfacing, no Node Power `queryEvents`, no broad polling, and no native browser popup.
- Validation passed: `npm run typecheck`; `npm run build`; explicit-env `npm run build`; `git diff --check` with only the preserved unrelated `contracts/civilization_control/Move.lock` LF/CRLF warning; `npx tsx scripts/check-node-power-presets.mts`; `npx tsx scripts/check-node-layout-overrides.mts`; `npx tsx scripts/check-structure-write-reconciliation.mts`; `npx tsx scripts/check-node-control-action-projection.mts`; `npx tsx scripts/check-operator-inventory-mapping.mts`; `npx tsx scripts/check-node-drilldown-reconciliation.mts`; `npx tsx scripts/check-network-node-offline-proof.mts`; `npx tsx scripts/check-node-power-batch-ptb.mts`; `npx tsx scripts/check-operator-inventory-status-truth.mts`; live `check-operator-inventory-status-truth` for the supplied wallet/object; `npm run sponsor:validate-policy`; `npm run sponsor:test`; `npm run sponsor:typecheck`; `sui client active-env` (`testnet_stillness`); `sui move build --path contracts/civilization_control`; and `sui move test --path contracts/civilization_control`. Move build/test only emitted the existing `gate_control.move` non-composable self-transfer lint warning.
- Preview proof is `https://8dd2583a.civilizationcontrol.pages.dev` with alias `https://feat-network-node-offline-pr-3px2.civilizationcontrol.pages.dev`; unique and alias `/`, `/nodes`, `/activity`, `/settings`, and `/dev/node-drilldown-lab` returned 200 and both hosts served `/assets/index-B0AqlrfE.js`.
- Served-bundle scanning across 12 preview JS assets found `civilizationcontrol-sponsor`, `https://civilizationcontrol-sponsor.michael-davis-home.workers.dev`, `https://ef-map.com`, `https://fullnode.testnet.sui.io:443`, `Already offline. View updated.`, and the targeted chain-status guard marker, and found no `flappy-frontier-sponsor`, exact-case `Authorization`, `ASSEMBLY_API_TOKEN`, `X-API-Key`, `SPONSOR_PRIVATE_KEY`, `CF_API_TOKEN`, or `CLOUDFLARE_ACCOUNT_ID`. Deployed lab browser smoke loaded the Node Control surface with local preset/bulk controls, `Power usage unavailable`, row-level `Bring online` / `Take offline`, and `Reset layout`.

### Node Power runtime correction - 2026-05-05

This corrective pass stays on `feat/network-node-offline-proof`. It does not change EF-Map contracts, VPS state, Move contracts, package IDs, vendor state, production deploy state, on-chain preset storage, Signal Feed parity, marketplace/revenue behavior, browser `queryEvents`, or broad polling.

- Human wallet smoke found two runtime defects in the local Node Power State slice: child rows could render `Online` even when the game/chain already treated them as offline, causing `status::offline` to abort with `EAssemblyInvalidStatus`; and `Take all offline`, `Bring all online`, and preset apply submitted grouped child writes instead of one wallet approval for the mixed child set.
- The transaction path now builds one mixed child-power PTB through `buildMixedAssemblyPowerTx(...)`. The PTB can mix generic `assembly`, `gate`, `storage_unit`, and `turret` child calls in one transaction, preserving one borrow -> action -> return cycle per target. Node Control calls `structurePower.toggleMixed(...)` once per bulk/preset plan instead of looping per family or per desired status.
- Before building the PTB, Node Control now refetches operator inventory and filters the plan against the freshest available child status. Children already in the requested state are dropped from the PTB, hidden-from-map children remain included when attached to the selected node, and the network-node anchor, other nodes, and unlinked structures remain excluded.
- `EAssemblyInvalidStatus` is now context-sensitive. When it comes from `status::offline` during an offline-intent write, CivilizationControl treats it as already-offline evidence for the affected child target, applies a digest-null offline correction overlay, reports `Already offline. View updated.`, and keeps stale online operator-inventory rows from immediately clearing that correction. The same abort is not swallowed for online-intent writes or `status::online` frames.
- `scripts/check-operator-inventory-status-truth.mts` proves raw offline rows adapt to offline, stale raw-online rows are locally corrected by already-offline evidence, stale raw-online data does not clear the correction overlay, and the supplied object hint can be inspected live with `--wallet <address> --object <objectId>`. The agent environment had the object hint `0xd2fd1224f881e7a705dbc211888af11655c315f2ee0f03fe680fc3176e6e4780` but no wallet address, so exact live EF-Map raw-row capture was skipped.
- WebSocket or push updates remain deferred. If operator-inventory is stale for many hours, a live transport alone would only stream stale truth faster; the durable follow-up is an EF-Map/operator-inventory status-indexing fix or source proof, then an optional wallet-scoped status-delta channel with bounded reconnect/backoff.
- Validation passed: `npm run typecheck`; `npm run build`; explicit-env `npm run build`; `git diff --check`; `npx tsx scripts/check-node-power-presets.mts`; `npx tsx scripts/check-node-layout-overrides.mts`; `npx tsx scripts/check-structure-write-reconciliation.mts`; `npx tsx scripts/check-node-control-action-projection.mts`; `npx tsx scripts/check-operator-inventory-mapping.mts`; `npx tsx scripts/check-node-drilldown-reconciliation.mts`; `npx tsx scripts/check-network-node-offline-proof.mts`; `npx tsx scripts/check-node-power-batch-ptb.mts`; `npx tsx scripts/check-operator-inventory-status-truth.mts`; `npm run sponsor:validate-policy`; `npm run sponsor:test`; `npm run sponsor:typecheck`; `sui client active-env` (`testnet_stillness`); `sui move build --path contracts/civilization_control`; and `sui move test --path contracts/civilization_control`. Move build/test only emitted the existing `gate_control.move` non-composable self-transfer lint warning.
- Preview proof is `https://6eb887e0.civilizationcontrol.pages.dev` with alias `https://feat-network-node-offline-pr-3px2.civilizationcontrol.pages.dev`; unique and alias `/`, `/nodes`, `/activity`, `/settings`, and `/dev/node-drilldown-lab` resolved cleanly and both hosts served `/assets/index-ySFU1fgs.js`.
- Served-bundle scanning across 12 preview JS assets found `civilizationcontrol-sponsor`, `https://civilizationcontrol-sponsor.michael-davis-home.workers.dev`, `https://ef-map.com`, `https://fullnode.testnet.sui.io:443`, and `Already offline. View updated.`, and found no `flappy-frontier-sponsor`, exact-case `Authorization`, `ASSEMBLY_API_TOKEN`, `X-API-Key`, `SPONSOR_PRIVATE_KEY`, `CF_API_TOKEN`, or `CLOUDFLARE_ACCOUNT_ID`. Deployed lab browser smoke loaded the Node Control surface with `Power usage unavailable`, row-level `Bring online` / `Take offline`, and `Reset layout`.
...
Corrective truth-source update (2026-05-04): human wallet smoke on a previous preview proved a read-model mismatch after chain already had the node offline: the UI still rendered the node as `Online`, and the next `Take offline` attempt failed in `world::network_node::offline` with `ENetworkNodeOffline`, which is authoritative evidence that runtime state was already offline. The displayed node status path is operator-inventory `networkNodes[].node.status` -> adapter `nodeGroups[].node.status` -> `/nodes`, node detail, and Node Control. The frontend does not invent `Online` from unknown state, so if the raw operator-inventory row says `online` while the Move precondition says offline, the indexed read model is stale or wrong. The repair stays frontend-local: `ENetworkNodeOffline` is now classified as already-offline evidence, applies the same offline overlay/refetch path without a digest, stale online operator-inventory data cannot immediately clear that correction, sponsored execution checks Move effects failure, and network-node action gating only exposes confident power actions for exact `online` or `offline` states. Connected child overlays are applied only when the existing offline plan supplied affected targets; node-online remains node-only. Selection Inspector `Freshness` was renamed to `Last Seen` because the value is `lastUpdated` falling back to `fetchedAt`, a mixed indexed/read-model observation timestamp rather than a freshness SLA.

- corrective validation passed: `npm run typecheck`; `npm run build`; `git diff --check` with only the preserved unrelated `contracts/civilization_control/Move.lock` LF/CRLF warning; `npx tsx scripts/check-network-node-offline-proof.mts`; `npx tsx scripts/check-structure-action-support.mts`; `npx tsx scripts/check-structure-write-reconciliation.mts`; `npx tsx scripts/check-node-control-action-projection.mts`; `npx tsx scripts/check-node-drilldown-reconciliation.mts`; `npx tsx scripts/check-operator-inventory-mapping.mts`; `npm run sponsor:validate-policy`; `npm run sponsor:test`; and `npm run sponsor:typecheck`. Move build/test were not rerun for this corrective pass because no transaction builder, Move code, package ID, or vendor code changed
- corrective preview proof is now `https://55d182a9.civilizationcontrol.pages.dev` with alias `https://feat-network-node-offline-pr-3px2.civilizationcontrol.pages.dev`; route smoke passed on `/`, `/nodes`, `/activity`, `/settings`, and `/dev/node-drilldown-lab`; unique and alias HTML both resolved `/assets/index-CXKQLW5D.js`; served-bundle scanning across 12 deployed JS assets found `civilizationcontrol-sponsor`, `https://civilizationcontrol-sponsor.michael-davis-home.workers.dev`, `https://ef-map.com`, `https://fullnode.testnet.sui.io:443`, `Node is already offline. View updated.`, and `Last Seen`, and found no `flappy-frontier-sponsor`, exact-case `Authorization`, `ASSEMBLY_API_TOKEN`, `X-API-Key`, `SPONSOR_PRIVATE_KEY`, `CF_API_TOKEN`, or `CLOUDFLARE_ACCOUNT_ID`
- deterministic proof now includes raw offline operator-inventory adaptation, `/nodes` and Node Control source parity, unknown or warning network-node action gating, `ENetworkNodeOffline` classification, digest-null already-offline overlays, stale-online operator-inventory protection, and a source guard proving `Freshness` is gone and `Last Seen` remains
- no EF-Map code, VPS state, Move code, package IDs, vendor state, presets, Signal Feed parity, marketplace/revenue work, browser `queryEvents`, broad polling, production deploy, or merge changed in this truth-source pass
...
- CivilizationControl now ships that minimum safe PTB through `buildNodeOfflineTx(...)`, backed by the live Stillness `FuelConfig` object `0x4fcf28a9be750d242bc5d2f324429e31176faecb5b84f0af7dff3a2a6e243550`; human wallet smoke proved the dedicated list/detail offline transaction succeeds and immediately powers the node down on-chain, then the focused branch repair replaced the native browser confirmation with an app modal, added the selected network-node self menu in Node Control, and locally projects the same node-offline consequence onto connected child structures
...
- Node Control now exposes network-node self actions from the selected node glyph, keyboard context-menu gesture, and node inspector surface: online nodes with a proven offline plan show `Take offline` plus `Rename Node`, offline nodes show `Bring online` plus `Rename Node`, and child structure menus keep their existing child action surface including `Hide from Node View`
- node-offline local reconciliation now creates one pending overlay for the network node and each connected child target resolved in the offline plan, applies those overlays to operator inventory before adaptation, keeps stale indexed online children from snapping back over a successful write, and clears overlays only when operator inventory confirms the target status; node-online remains node-only and does not mark children online
- validation passed after the corrective repair: `npm run typecheck`; `npm run build`; `npx tsx scripts/check-network-node-offline-proof.mts`; `npx tsx scripts/check-structure-action-support.mts`; `npx tsx scripts/check-structure-write-reconciliation.mts`; `npx tsx scripts/check-node-control-action-projection.mts`; `npx tsx scripts/check-node-drilldown-reconciliation.mts`; `npm run sponsor:validate-policy`; `npm run sponsor:test`; `npm run sponsor:typecheck`; `sui client active-env` (`testnet_stillness`); `sui move build --path contracts/civilization_control`; and `sui move test --path contracts/civilization_control`
- fresh corrective preview proof is now `https://e90e9431.civilizationcontrol.pages.dev` with alias `https://feat-network-node-offline-pr-3px2.civilizationcontrol.pages.dev`; unique and alias `/`, `/nodes`, `/activity`, `/settings`, and `/dev/node-drilldown-lab` all resolved the same entry chunk `/assets/index-BHriBt8c.js`; served-bundle scanning found `civilizationcontrol-sponsor` plus `https://civilizationcontrol-sponsor.michael-davis-home.workers.dev` in `SmartObjectProvider-Da1AMNkr.js` and `App-Bn-mbloQ.js`, found `https://ef-map.com` in `SmartObjectProvider-Da1AMNkr.js` and `nodeDrilldownMenuItems-Bp7QuEak.js`, found the app confirmation copy and node rename copy in deployed assets, and found no `flappy-frontier-sponsor`, `ASSEMBLY_API_TOKEN`, `X-API-Key`, exact-case `Authorization`, `SPONSOR_PRIVATE_KEY`, `CF_API_TOKEN`, or `CLOUDFLARE_ACCOUNT_ID`
- deployed lab smoke on the fresh preview confirmed the selected node glyph opens a node self menu with `Take offline` / `Rename Node`; taking the synthetic node offline marks the node plus connected visible and hidden children offline; bringing the node online marks only the node online; and child row right-click still exposes `Hide from Node View`, child power, and `Rename Assembly`
- integrated-browser wallet smoke for a live owned node remains unavailable in this environment, but the branch repair is grounded in the prior human wallet proof that the dedicated list transaction succeeds, plus deterministic PTB, reconciliation, Node Control projection, route, served-bundle, and deployed-lab proof
...
- the shared structure action context menu now shrink-wraps through a max-content grid and measured viewport clamp, so the longest current labels (`Hide from Node View`, `Bring Online`, `Take Offline`, `Rename Assembly`, `Unhide`) fit without the earlier wide right-side dead space. Fresh preview smoke measured the normal three-item menu at about `156px` wide with `10px` left/right padding around the longest labels; hidden-row menus still size to the longest row label instead of a fixed stale width
- the `Attached Structures` panel now keeps its mobile/tablet cap but increases the desktop scroll viewport through `lg:max-h-[min(76vh,720px)]`, preserving the internal scrollbar for dense nodes without shrinking the map. Fresh preview stress smoke measured the Turret Stress Test list viewport at `698px` with a `3026px` scroll height, instead of the previous short desktop cap
- the remaining human-observed Signal Feed issue is documented as parity only: app-originated rename and power writes reflected immediately in Dashboard and Node Control, but those same writes did not appear in `/activity`. This branch does not restore browser `queryEvents` and does not implement history expansion; future work must first determine whether EF-Map signal-history already indexes those exact rows and CivilizationControl filters them out, or whether EF-Map needs additive coverage
- validation passed after the final patch: `git diff --check`; `npm run typecheck`; `npm run build` with explicit public `VITE_SPONSOR_URL`, `VITE_SHARED_BACKEND_URL`, blank `VITE_SPONSOR_API_KEY`, and `VITE_SUI_RPC_URL`; `npx tsx scripts/check-operator-inventory-mapping.mts`; `npx tsx scripts/check-node-drilldown-reconciliation.mts`; `npx tsx scripts/check-signal-history-mapping.mts`; `npx tsx scripts/check-structure-action-support.mts`; `npx tsx scripts/check-structure-write-reconciliation.mts`; `npx tsx scripts/check-node-control-action-projection.mts`; `npx tsx scripts/check-node-drilldown-lab-action-matrix.mts`; `npm run sponsor:validate-policy`; and `npm run sponsor:test`. Move build/test were not rerun because this pass did not change transaction builders, contracts, package IDs, or Move state
- final preview proof is `https://355b5c25.civilizationcontrol.pages.dev` with alias `https://feat-zero-package-action-par-da22.civilizationcontrol.pages.dev`; route smoke confirmed `/`, `/nodes`, `/activity`, `/settings`, and `/dev/node-drilldown-lab`; UI smoke confirmed compact Node Control map/row/generic/hidden menus and the taller dense Attached Structures list; served-bundle scanning across 12 deployed JS assets found `civilizationcontrol-sponsor`, the repo-owned sponsor URL, `https://ef-map.com`, and the explicit Sui RPC URL in expected chunks, and found no `flappy-frontier-sponsor`, exact-case `Authorization`, `ASSEMBLY_API_TOKEN`, `X-API-Key`, `SPONSOR_PRIVATE_KEY`, `CF_API_TOKEN`, or `CLOUDFLARE_ACCOUNT_ID`
- integrated-browser wallet smoke was attempted on the final preview, but a usable wallet-owned session was still unavailable: the connect modal exposed only `Slush`, and the connection attempt immediately returned `Connection failed`
...
- human wallet closeout later proved the remaining write path: app-originated rename and power writes now reflect immediately in Dashboard and Node Control through the local overlay plus operator-inventory refresh path, but those same writes do not currently appear in `/activity`. Treat that as a Signal Feed parity gap, not a write-path, package, sponsor, or Node Control authority blocker. This branch should not restore browser `queryEvents` or implement history expansion; future work must determine whether EF-Map signal-history already indexes those exact rename/power rows and CivilizationControl filters them out, or whether EF-Map needs additive signal-history coverage
...
4. Signal-history parity remains separate
  ...
  - include app-originated rename/power writes in that parity investigation: first compare the EF-Map signal-history rows for the relevant tx digests against CivilizationControl filtering before requesting any additive backend expansion
