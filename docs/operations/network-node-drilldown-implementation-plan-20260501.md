...
- The requested `Power 600 / 1000 GJ` meter is deferred as an exact meter because current frontend/operator-inventory data exposes node fuel/runtime, child status, child type ID, energy-source ID, and write target IDs, but not `EnergyConfig.assembly_energy` or `NetworkNode.energy_source.{max_energy_production,current_energy_production,total_reserved_energy}`. Normal Node Control therefore renders the calm readout `Power usage unavailable` instead of fabricated GJ math.
- The 1000 GJ capacity constant is recorded for the product rule, but exact over-cap blocking is not active because the app cannot prove child power requirements from current data. Online bulk/preset operations that would bring children online use an app-styled confirmation explaining that power requirements are unavailable; chain execution remains the final enforcement. Take-all-offline needs no capacity precheck.
- Follow-up for exact meter/cap enforcement: expose indexed node `maxEnergyProductionGJ`, `currentEnergyProductionGJ`, `totalReservedEnergyGJ`, and per-child `energyRequiredGJ` resolved by type ID from `EnergyConfig`, with source/freshness metadata, through operator-inventory or an existing mapped indexed field. Do not infer power from names or hardcode family values in the frontend.
- Deterministic proof added: `scripts/check-node-power-presets.mts` and `scripts/check-node-layout-overrides.mts` cover child-only bulk targeting, hidden-row inclusion, preset diff/mismatch behavior, scoped storage keys, capacity-unavailable behavior, local overlay target propagation, layout coordinate persistence, clamping, hidden/unhidden layout preservation, and reset-layout scope.
- Validation passed: `npm run typecheck`; `npm run build`; explicit-env preview `npm run build` with `VITE_SPONSOR_URL=https://civilizationcontrol-sponsor.michael-davis-home.workers.dev`, `VITE_SHARED_BACKEND_URL=https://ef-map.com`, blank `VITE_SPONSOR_API_KEY`, and `VITE_SUI_RPC_URL=https://fullnode.testnet.sui.io:443`; `git diff --check` with only the preserved unrelated `contracts/civilization_control/Move.lock` LF/CRLF warning; all requested Node Control/operator-inventory/signal deterministic probes; `scripts/check-node-power-presets.mts`; `scripts/check-node-layout-overrides.mts`; `npm run sponsor:validate-policy`; `npm run sponsor:test`; and `npm run sponsor:typecheck`. Move build/test were not rerun because no transaction builder, Move contract, package-ID, or vendor code changed.
- Fresh preview proof is `https://70c1585a.civilizationcontrol.pages.dev` with alias `https://feat-network-node-offline-pr.civilizationcontrol.pages.dev`; route smoke passed on `/`, `/nodes`, `/activity`, `/settings`, and `/dev/node-drilldown-lab`; deployed dev-lab smoke confirmed the unavailable power readout, drag persistence, reset-layout scope, right-click menu, hide/unhide, and selection inspector behavior; served-bundle scanning found the repo sponsor URL plus `https://ef-map.com` and found no stale sponsor, exact-case `Authorization`, or checked secret/header markers. Wallet smoke remained unavailable in the agent browser because no usable wallet-backed session was present.
