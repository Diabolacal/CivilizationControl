...
- limitation: a connected-wallet preview session was not available in this environment, so browser proof of live wallet-scoped signal rows, category filtering, and cursor pagination remains pending even though the endpoint contract and disconnected route behavior are now validated
