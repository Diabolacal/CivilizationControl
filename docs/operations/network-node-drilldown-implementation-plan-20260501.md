...
### Node Control authority regression repair - 2026-05-06

This corrective pass stays on `fix/node-control-authority-regression`. It does not change EF-Map or VPS contracts, Move contracts, package IDs, sponsor-worker behavior, browser `queryEvents`, broad polling, or production deploy state.

- Manual wallet smoke on `master` after the `feat/signal-history-display-parity` merge exposed a frontend regression in Node Control only: attached-structure and node-self write actions disappeared, child bulk power controls disabled, `Power usage unavailable` reappeared, and warm entry into the operator shell plus Node Control felt slower even though global Turrets and Storages actions still worked.
- Root cause was frontend duplicate-row preference, not a backend contract change. `src/lib/operatorInventoryAdapter.ts` started letting display-fresher duplicate node or child rows win wholesale even when those rows were weaker than the original operator-inventory row and lacked `ownerCapId`, action-candidate authority, wrapper-level `powerUsageSummary`, or child `powerRequirement` data. In backend-membership Node Control, the selected-node view then lost the verified action target and power inputs even though operator inventory still had them.
- The fix keeps operator inventory authoritative for selected-node write authority and power controls. Duplicate node and child rows now merge field-by-field: fresher `displayName` or status can enrich presentation, but `ownerCapId`, action candidates, node `powerUsageSummary`, child `powerRequirement`, and related write metadata remain sticky from the stronger operator-inventory-backed row.
- Deterministic proof now covers the exact regression shape. `scripts/check-operator-inventory-mapping.mts` includes duplicate node plus duplicate child fixtures that fail if fresher fallback rows strip action authority or power data, and `scripts/check-node-power-live-usage.mts` now fails if live operator-inventory authority is lost in the selected-node model.
- Live validation for wallet `0x11dd567e72d160ad7116a7358684dfff800af2a8e429cd1a65778640f8a61f62` showed the repaired selected-node path staying on `backend-membership` with zero authority failures, `Power 770 / 1000 GJ` on the active node, `Power 0 / 1000 GJ` on the offline node, enabled `Take all offline` on the first node, enabled `Bring all online` on the second node, and enabled preset save on both nodes.
- Performance findings stayed narrow. Strategic Network still renders from root `useAssetDiscovery` data and is not gated by Signal Feed, while `useNodeAssemblies(...)` remains fallback-only when the selected node has no operator-inventory lookup and the operator-inventory query is already in error. The concrete hot-path slowdown repaired here was forced operator-inventory mount refetch on warm cache; `useOperatorInventory()` now uses stale-aware `refetchOnMount: true` instead of forced `"always"`.
- Fresh preview proof is `https://ac0cd430.civilizationcontrol.pages.dev` with alias `https://fix-node-control-authority-r.civilizationcontrol.pages.dev`. Unique and alias `/`, `/activity`, `/nodes`, `/settings`, and `/dev/node-drilldown-lab` all returned `200`, both hosts served `/assets/index-Cwhp3oOh.js`, and served-bundle scanning across all 12 deployed JavaScript assets found `https://civilizationcontrol-sponsor.michael-davis-home.workers.dev` plus `https://ef-map.com` while finding no `flappy-frontier-sponsor`, `ASSEMBLY_API_TOKEN`, `X-API-Key`, `SPONSOR_PRIVATE_KEY`, `CF_API_TOKEN`, or `CLOUDFLARE_ACCOUNT_ID`. The only exact-case `Authorization` bundle hits remained user-facing `extension authorization` copy, not browser auth-header behavior.
- Deployed browser smoke on `/dev/node-drilldown-lab` loaded `Node Drilldown Lab`, rendered `Power 320 / 1000 GJ`, showed visible attached-structure action rails, exposed right-click `Hide from Node View`, `Bring Online`, and `Rename Assembly` for `Printer Delta`, and confirmed those row/menu controls carry `title: null`. Wallet-connected preview smoke was not available in this environment because the integrated browser still exposed only `Slush`, which failed with `Connection failed`.
- Remaining known gaps stay unchanged and separate from this regression fix: historical rename rows are not backfilled into current Signal Feed proof, fresh CivilizationControl custom-package rows still need live action proof when new writes exist, gate-specific live smoke remains weak until real gate rows exist on the test wallet, and external marketplace coverage is still deferred.
